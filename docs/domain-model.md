...
| Transaction | Nachvollziehbare erfolgreich gebuchte Geldbewegung | `id`, `receipt_reference` (eindeutig), `account_id`, optional `card_id`, optional `atm_id`, `type`, optional `purpose`, `amount_minor`, `currency`, `balance_after_minor`, `idempotency_key` (eindeutig je Konto), optional `cash_breakdown`, `created_at` | Positiver Betrag; Vorzeichenwirkung ergibt sich aus `withdrawal`/`deposit`. Verwendungszweck maximal 140 Zeichen. Erfolgreiche Buchungen unveränderlich; Korrekturen später als Gegenbuchung. Abgewiesene Versuche erzeugen keine Buchung, aber bei einer geprüften Geschäftsregel ein AuditEvent. |
...
- Der Eloquent-Änderungsschutz des Audits ist keine manipulationssichere Archivierung. Direkte SQL-Zugriffe oder ein kompromittierter Datenbankzugang können ihn umgehen; Aufbewahrung und Export sind noch offen.
